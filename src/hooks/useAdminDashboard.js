...
export const useAdminDashboard = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    refetchInterval: 120000, // 2 min polling (socket handles instant updates)
    staleTime: 2 * 60 * 1000, // 2 min — socket events cover real-time freshness
  ...
  const deactivateMutation = useMutation({
    mutationFn: ({ userId, status }) =>
      AdminService.deactivateUser(userId, status),
    onSuccess: () => {
      return queryClient.invalidateQueries({ queryKey: ['admin-dashboard'] });
    },
  });
  ...
  const updateTeamMutation = useMutation({
    mutationFn: ({ teamId, teamData }) =>
      AdminService.updateTeam(teamId, teamData),
    onSuccess: () => {
      return queryClient.invalidateQueries({ queryKey: ['admin-dashboard'] });
    },
  });
  ...
  const createRoleMutation = useMutation({
    mutationFn: ({ roleName, permissions }) =>
      AdminService.createRole(roleName, permissions),
    onSuccess: () => {
      return queryClient.invalidateQueries({ queryKey: ['admin-dashboard'] });
    },
  });
  ...
  return {
    ...
    updateTeam: (teamId, teamData) =>
      updateTeamMutation.mutateAsync({ teamId, teamData }),
    deleteTeam: (teamId) => deleteTeamMutation.mutateAsync(teamId),
    updateRolePermissions: (payload) =>
      updateRolePermissionsMutation.mutateAsync(payload),
