...
// Facilitates user customization of the application interface, including dark/light mode and accent colors.
const Theme = () => {
    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="mb-12">
        ...
        <p className="text-text-muted text-xs font-medium mt-3 max-w-md opacity-50">
          Customize your workspace appearance with our professional theme
          presets and accent colors.
        </p>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {themes.map((t) => (
                ...
                >
                  ...
                  <div
                    className={`w-16 h-12 rounded-xl mb-6 shadow-xl ${t.preview}`}
                  >
          ...
          <div className="bg-[#121826]/30 backdrop-blur-3xl border border-white/5 rounded-[40px] p-8 md:p-10 shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-wrap gap-4">
              {accents.map((acc) => (
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl shadow-inner ${acc.color} flex items-center justify-center text-white`}
                  >
            ...
            <div className="mt-8 p-6 bg-white/[0.02] border border-white/5 rounded-3xl">
              <div className="text-[9px] font-black text-white/20 uppercase tracking-[0.2em] mb-3">
                Preview
              </div>
              <div className="flex items-center gap-4">
                <button
                  className={`px-4 py-2 bg-${accentColor}-500 text-white rounded-lg text-[10px] font-black uppercase tracking-widest shadow-lg shadow-${accentColor}-500/20`}
                >
                  Button Action
                </button>
                <div
                  className={`px-4 py-2 border border-${accentColor}-500/30 text-${accentColor}-500 rounded-lg text-[10px] font-black uppercase tracking-widest`}
                >
                  Sample Text
                </div>
                <div className="flex-1 h-1 bg-white/5 rounded-full overflow-hidden">
                  <div
                    className={`h-full bg-${accentColor}-500 w-3/4 shadow-[0_0_10px_rgba(var(--color-primary),0.5)]`}
                  ></div>
                </div>
              </div>
            </div>
...
const SyncOption = ({ icon, title, Active }) => (
  ...
  >
    ...
    <div
      className={`w-4 h-4 rounded-full border flex items-center justify-center ${Active ? 'border-primary bg-primary' : 'border-white/10'}`}
    >
