...
const SEDashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-background text-text p-4 lg:p-6 font-sans selection:bg-primary max-w-screen-2xl mx-auto flex flex-col gap-6">
      ...
      <div id="se-metrics-strip" className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <MetricStripItem 
            label="Assigned Load" 
                ...
                <tbody className="divide-y divide-white/[0.02] text-[10px] font-black uppercase tracking-widest">
                   {workQueue?.map((task, idx) => (
                    <tr key={idx} className="group hover:bg-white/[0.015] transition-colors cursor-pointer" onClick={() => task.id && navigate(`/task/${task.id}`)}>
                      ...
                      <td className="py-3 px-4">
                        <span className={`text-[9px] font-black tracking-[0.2em] ${task.isOverdue ? 'text-status-error' : 'text-white/40'}`}>
                          {task.due}
                        </span>
                      </td>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 py-2">
                {teamSupport?.map((help, idx) => (
                  ...
                  >
                     ...
                     <p className="text-[9px] font-black text-white uppercase leading-snug tracking-widest line-clamp-2">{help.issue}</p>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <DashboardSection title="Operational Blockers" icon={<ShieldAlert size={14} />}>
             <div className="flex flex-col gap-2 py-2">
                ...
                {!myBlockers?.length && (
                  <div className="py-8 text-center text-[9px] text-white/10 uppercase font-black tracking-[0.2em] italic">ZERO_BLOCKADE_STATE</div>
                )}
             </div>
          </DashboardSection>
          ...
          <DashboardSection title="Sync Integrity" icon={<Target size={14} />}>
             <div className="flex items-center justify-around py-4 mt-2 bg-white/5 rounded border border-white/10">
                <div className="flex flex-col items-center gap-1">
                  <span className="text-2xl font-black text-white tracking-widest">{activity.commitsToday}</span>
                  <span className="text-[8px] font-black text-white/20 uppercase tracking-[0.2em]">COMMITS</span>
                </div>
                <div className="w-px h-8 bg-white/10" />
                <div className="flex flex-col items-center gap-1">
                  <span className="text-2xl font-black text-white tracking-widest">{activity.filesChanged}</span>
                  <span className="text-[8px] font-black text-white/20 uppercase tracking-[0.2em]">CHANGES</span>
                </div>
                <div className="w-px h-8 bg-white/10" />
                <div className="flex flex-col items-center gap-1">
                  <span className="text-2xl font-black text-white tracking-widest">{activity.prsMerged}</span>
                  <span className="text-[8px] font-black text-white/20 uppercase tracking-[0.2em]">MERGED</span>
                </div>
             </div>
          </DashboardSection>
        </div>
        ...
        <div className="lg:col-span-8">
           <DashboardSection title="Structural Defect Queue" icon={<Bug size={14} />}>
              <div className="flex flex-col gap-2 py-2">
                 {myBugs?.map((bug, idx) => (
                    ...
                    >
                       <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col gap-1">
                             <span className="text-[10px] font-black text-white uppercase tracking-widest group-hover:text-status-error transition-colors leading-none mb-1">{bug.issue}</span>
                             <span className="text-[8px] text-white/40 uppercase font-black tracking-[0.2em] leading-none">{bug.severity} SEVERITY</span>
                          </div>
                       </div>
                       <div className="flex items-center gap-4">
                 ...
                 {!myBugs?.length && (
                  <div className="py-8 text-center text-[9px] text-white/10 uppercase font-black tracking-[0.2em] italic">ZERO_DEFECT_ENVIRONMENT</div>
                )}
              </div>
           </DashboardSection>
        </div>
