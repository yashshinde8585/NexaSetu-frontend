...
const MetricsDashboard = () => {
  ...
  if (!hasPermission) return <div className="p-4 bg-red-100 text-red-700">Access Denied: You do not have permission to view the metrics dashboard.</div>;
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-white p-4 rounded shadow">
          ...
          <p>Status: <span className={`font-bold ${health.status === 'HEALTHY' ? 'text-green-600' : 'text-yellow-600'}`}>{health.status}</span></p>
          <p>Database: {health.database}</p>
        </div>
        
        <div className="bg-white p-4 rounded shadow">
          <h2 className="text-xl font-semibold mb-2">Recent Execution Metrics</h2>
          {metrics && metrics.length > 0 ? (
            <ul>
              {metrics.slice(0, 5).map((m, idx) => (
                <li key={idx} className="border-b py-2">
                  <span className="capitalize font-medium">{m.type.replace('_', ' ')}</span>: {m.value}
                </li>
              ))}
            </ul>
          ) : (
