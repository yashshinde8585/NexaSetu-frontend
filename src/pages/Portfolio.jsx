...
const Portfolio = () => {
  ...
  const escalations = projects.filter(p => p.riskLevel === 'High' || p.healthScore < 50);
  const portfolioHealth = Math.round((projects.filter(p => p.riskLevel === 'Low').length / projects.length) * 100) || 0;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-12 space-y-12 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex-1">
          <h1 className="text-4xl font-black text-white tracking-tighter mb-2">Portfolio Orchestration</h1>
          <p className="text-text-muted text-xs font-bold uppercase tracking-[0.2em] opacity-60">Aggregate Strategic Control Layer</p>
        </div>
        {hasPermission(PERMISSIONS.GENERATE_EXEC_BRIEF) && (
          <button
            ...
            onClick={() => alert("NexaSetu AI is analyzing 428 commits, 12 project delays, and team velocity to generate your briefing...")}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-background-light/20 border border-white/5 p-8 rounded-[2.5rem] backdrop-blur-xl relative overflow-hidden group">
          <div className="text-[10px] font-black text-text-muted/60 uppercase tracking-widest mb-4">Portfolio Health</div>
          <div className="flex items-end gap-3">
            <span className="text-5xl font-black text-white">{portfolioHealth}%</span>
            <span className={`text-xs font-bold ${portfolioHealth > 70 ? 'text-status-success' : 'text-status-error'} mb-2`}>
              {portfolioHealth > 70 ? '🟢 OPTIMAL' : '🔴 CRITICAL'}
            </span>
          </div>
          <div className="mt-6 h-1 w-full bg-white/5 rounded-full">
            <div className="h-full bg-status-success" style={{ width: `${portfolioHealth}%` }}></div>
          </div>
        </div>

        <div className="bg-background-light/20 border border-white/5 p-8 rounded-[2.5rem] backdrop-blur-xl relative overflow-hidden group">
          <div className="text-[10px] font-black text-text-muted/60 uppercase tracking-widest mb-4">Global Delivery Velocity</div>
          <div className="flex items-end gap-3">
            <span className="text-5xl font-black text-white">{globalVelocity}%</span>
            <span className="text-xs font-bold text-status-info mb-2">🚀 +12% MoM</span>
          </div>
          <div className="mt-6 h-1 w-full bg-white/10 rounded-full">
            <div className="h-full bg-status-info" style={{ width: `${globalVelocity}%` }}></div>
          </div>
        </div>

        <div className="bg-background-light/20 border border-white/5 p-8 rounded-[2.5rem] backdrop-blur-xl relative overflow-hidden group">
          <div className="text-[10px] font-black text-text-muted/60 uppercase tracking-widest mb-4">Active Escalations</div>
          <div className="flex items-end gap-3">
            <span className="text-5xl font-black text-status-error">{escalations.length}</span>
            <span className="text-[10px] font-black text-text-muted uppercase tracking-tighter mb-2">Projects at Risk</span>
          </div>
          <div className="mt-6 flex gap-1">
            {escalations.map((_, i) => <div key={i} className="w-4 h-1 bg-status-error rounded-full"></div>)}
            {escalations.length === 0 && <div className="text-[10px] font-bold text-status-success italic">NO CRITICAL FIRES DETECTED</div>}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-8">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-sm font-black uppercase tracking-[0.2em] text-status-error">Critical High-Risk Escalations</h2>
        </div>
        {escalations.length === 0 ? (
          <div className="bg-background-light/40 border-2 border-dashed border-white/10 rounded-[3rem] p-16 flex flex-col items-center justify-center text-center">
            <h3 className="text-xl font-bold text-status-success italic">All Projects are within Optimal Execution Boundaries</h3>
          </div>
        ) : (
