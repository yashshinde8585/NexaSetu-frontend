...
const Settings = () => {
    ...
    const tabs = [
        { id: 'identity', label: 'Profile Identity', icon: <User size={18} /> },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-10 py-6 sm:py-10 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="mb-12">
                ...
                <p className="text-text-muted text-[10px] sm:text-xs font-medium mt-3 max-w-md opacity-50">
                    Customize your profile, account security, and notification preferences.
                </p>
            </div>
                ...
                <div className="w-full lg:w-72 flex flex-col gap-2">
                    ...
                    <div className="mt-6 p-5 sm:p-6 bg-linear-to-br from-primary/5 to-transparent border border-white/5 rounded-2xl sm:rounded-3xl">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-[8px] font-black text-white/60 uppercase tracking-widest">Current Role</span>
                        </div>
                        <div className="text-[10px] font-bold text-white uppercase tracking-tight line-clamp-1">
    ...
    <DeckWrapper title="Profile Identity">
        <div className="flex flex-col md:flex-row items-center gap-6 sm:gap-10 p-5 sm:p-6 bg-white/[0.02] rounded-2xl sm:rounded-3xl border border-white/5 group hover:bg-white/[0.04] transition-all duration-500">
            ...
            <div className="flex-1 space-y-2 text-center md:text-left">
                <div className="text-[9px] font-black text-white/20 uppercase tracking-[0.2em]">Full Name</div>
                <h3 className="text-2xl font-extrabold text-white uppercase tracking-tight">{user.name}</h3>
                <p className="text-xs text-text-muted opacity-60">Upload a profile picture to help your team members identify you.</p>
            </div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8">
            <TacticalInput icon={<UserCircle />} label="Full Name" value={user.name} placeholder="John Doe" />
            <TacticalInput icon={<AtSign />} label="Email Address" value={user.email} placeholder="john@nexus.app" />
            <TacticalInput icon={<BadgeCheck />} label="Account Role" value={user.jobTitle || 'Team Member'} disabled />
            <TacticalInput icon={<Clock />} label="Member Since" value={`Joined ${new Date(user.createdAt).toLocaleDateString()}`} disabled />
        </div>
    </DeckWrapper>
);
