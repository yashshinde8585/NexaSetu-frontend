...
const Dashboard = () => {
  ...
  const stats = React.useMemo(() => {
    ...
    if (isAdmin) {
      ...
      computedStats = [
        { label: 'To-Do', value: personal.todo, color: 'bg-white/5 border-white/10 text-white/40', icon: <FolderOpen size={20} />, trend: `Overall: ${summary.todo}`, to: '/my-tasks?filter=todo&scope=personal' },
        { label: 'In Progress', value: personal.in_progress, color: 'bg-primary/10 border-primary/20 text-primary', icon: <Rocket size={20} />, trend: `Overall: ${summary.in_progress}`, to: '/my-tasks?filter=in_progress&scope=personal' },
        { label: 'In Review', value: personal.in_review, color: 'bg-secondary/10 border-secondary/20 text-secondary', icon: <ShieldCheck size={20} />, trend: `Overall: ${summary.in_review}`, to: '/my-tasks?filter=in_review&scope=personal' },
        { label: 'Done', value: personal.done, color: 'bg-status-success/10 border-status-success/20 text-status-success', icon: <CheckCircle size={20} />, trend: `Overall: ${summary.done}`, to: '/my-tasks?filter=completed&scope=personal' },
      ];
    }
    ...
    <div className="max-w-screen-2xl mx-auto px-4 sm:px-8 lg:px-10 pt-8 pb-12 space-y-12">
      ...
      <div className="space-y-12">
        ...
        <div className="relative z-20">
          <RoleAnalytics
            user={user}
            projects={selectedProjectId ? projects.filter(p => p._id === selectedProjectId) : projects}
            workload={(() => {
              if (!selectedProjectId) return workload;
              const members = projects.find(p => p._id === selectedProjectId)?.members || [];
              const memberNames = members.map(m => typeof m === 'string' ? m : (m.name || m._id));
