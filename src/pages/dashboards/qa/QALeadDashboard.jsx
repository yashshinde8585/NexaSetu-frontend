...
const QALeadDashboard = () => {
  ...
  const {
    ...
    releaseDecision = {
      ...
      reason: 'UNDEFINED',
      responsibleTeams: [],
      metrics: { criticalBugs: 0, highFailures: 0, blockedTests: 0 },
    },
    crossTeamBlockers = [],
    criticalWatchlist = [],
    teamSignals = [],
    qualityTrend = {
      passRate: { now: 0, last: 0 },
      bugCount: { now: 0, last: 0 },
    },
    coverageOverview = [],
    regressionHealth = {
      passed: 0,
    ...
    <div className="min-h-screen bg-background text-text p-4 lg:p-6 font-sans selection:bg-primary max-w-screen-2xl mx-auto flex flex-col gap-6">
      ...
      >
        <MetricStripItem
          ...
          icon={<Flag size={14} />}
          color={
            globalControl.releaseStatus === 'READY'
              ? 'text-status-success'
              : 'text-status-error'
          }
          accent={
        ...
        <MetricStripItem
          ...
          icon={<Users size={14} />}
          color={
            globalControl.teamsAtRisk > 0
              ? 'text-status-warning'
              : 'text-white/40'
          }
          accent={
            globalControl.teamsAtRisk > 0 ? 'bg-status-warning' : 'bg-white/5'
          }
        />
        <MetricStripItem
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <DashboardSection
            title="Structural Quality Matrix"
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/[0.02]">
                  {teamOverview?.map((t, idx) => (
                    <tr
                      key={idx}
                      className="group hover:bg-white/5 transition-colors text-white/80"
                    >
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-1.5 h-1.5 rounded-none ${t.status === 'red' ? 'bg-status-error' : 'bg-status-success'}`}
                          />
                      ...
                      <td className="py-3 px-3">
                        <div className="flex flex-col gap-1.5 w-32">
                          <span
                            className={`text-[9px] font-black ${t.passRate < 80 ? 'text-status-error' : 'text-status-success'}`}
                          >
                            {t.passRate}%
                          </span>
                          <div className="w-full h-0.5 bg-white/5 rounded-none overflow-hidden">
                            <div
                              className={`h-full ${t.passRate < 80 ? 'bg-status-error' : 'bg-status-success'}`}
                              style={{ width: `${t.passRate}%` }}
                            />
                          </div>
                        </div>
                      </td>
                      <td className="py-3 px-3 text-center text-[10px] font-black tabular-nums">
                        {t.bugs}
                      </td>
                      <td className="py-3 px-3 text-center text-[10px] font-black tabular-nums">
                        {t.blockers}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <DashboardSection
              title="Structural Defect Trends"
              icon={<BarChart3 size={14} />}
            >
              <div className="flex flex-col gap-6 py-4 px-2">
                <TrendMetric
                  label="AGGREGATE_PASS_RATE"
                  now={qualityTrend.passRate.now}
                  last={qualityTrend.passRate.last}
                  unit="%"
                  upIsGood
                />
                <TrendMetric
            ...
            >
              <div className="flex flex-col gap-2 py-2">
                {crossTeamBlockers?.map((b, idx) => (
                  <div
                    key={idx}
                    className="p-3 bg-white/5 border border-white/10 rounded-none flex gap-4 items-start group hover:bg-white/10 transition-colors"
                  >
                    ...
                    <div className="ml-auto">
                      <span className="text-[8px] font-black italic text-status-warning uppercase tracking-[0.2em]">
                        {b.severity}
                      </span>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          <DashboardSection
            title="Release Decision Center"
            icon={<ShieldCheck size={14} />}
          >
            <div
              className={`mt-2 flex flex-col items-center py-4 rounded-none border ${releaseDecision.isHold ? 'border-status-error/20 bg-status-error/[0.02]' : 'border-status-success/20 bg-status-success/[0.02]'}`}
            >
              <div
                className={`text-2xl font-black tracking-widest mb-2 ${releaseDecision.isHold ? 'text-status-error' : 'text-status-success'}`}
              >
              ...
              <div className="grid grid-cols-2 gap-2 w-full px-4">
                {releaseDecision.responsibleTeams.map((team, idx) => (
                  <div
                    key={idx}
                    className="px-2 py-1 bg-white/5 border border-white/10 rounded-none text-[8px] font-black text-white/60 uppercase tracking-[0.2em] text-center"
                  >
                    {team}_DOMAIN
                  </div>
                ))}
            ...
            <div className="grid grid-cols-3 gap-2 mt-4 px-1">
              <div className="flex flex-col items-center justify-center p-3 bg-white/5 border border-white/10 rounded-none">
                <span className="text-lg font-black text-status-error leading-none">
                  {releaseDecision.metrics.criticalBugs}
                </span>
                <span className="text-[7px] font-black text-white/20 uppercase tracking-[0.2em] mt-2">
                  CRITICAL
                </span>
              </div>
              <div className="flex flex-col items-center justify-center p-3 bg-white/5 border border-white/10 rounded-none">
                <span className="text-lg font-black text-status-error leading-none">
                  {releaseDecision.metrics.highFailures}
                </span>
                <span className="text-[7px] font-black text-white/20 uppercase tracking-[0.2em] mt-2">
                  FAILURES
                </span>
              ...
              <div className="flex flex-col items-center justify-center p-3 bg-white/5 border border-white/10 rounded-none">
                <span className="text-lg font-black text-status-warning leading-none">
                  {releaseDecision.metrics.blockedTests}
                </span>
                <span className="text-[7px] font-black text-white/20 uppercase tracking-[0.2em] mt-2">
                  BLOCKED
                </span>
            ...
            <div className="flex flex-col gap-2 py-2">
              {criticalWatchlist?.map((bug, idx) => (
                ...
                >
                  ...
                  <ChevronRight
                    size={12}
                    className="text-white/10 group-hover:text-status-error transition-colors"
                  />
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <DashboardSection
          title="Structural Coverage Distribution"
          icon={<Layers size={14} />}
        >
            ...
            {coverageOverview?.map((cov, idx) => (
              <div key={idx} className="flex flex-col gap-2">
                <div className="flex justify-between items-center leading-none">
                  ...
                  <span
                    className={`text-[10px] font-black ${cov.status === 'stable' ? 'text-status-success' : 'text-status-error'}`}
                  >
                    {cov.coverage}%
                  </span>
                </div>
                <div className="w-full h-0.5 bg-white/5 rounded-none overflow-hidden">
                  <div
                    className={`h-full transition-all duration-1000 ${cov.status === 'stable' ? 'bg-status-success' : 'bg-status-error'}`}
        ...
        <DashboardSection
          title="Aggregate Regression Stability"
              ...
              <div className="flex-1 flex flex-col gap-4">
                <div className="flex justify-between text-[9px] font-black uppercase tracking-[0.2em] leading-none">
                  <span className="text-status-error">
                    FAIL: {regressionHealth.failed}
                  </span>
                  <span className="text-status-warning">
                    PEND: {regressionHealth.pending}
                  </span>
                </div>
                <div className="w-full h-1 bg-white/5 rounded-none overflow-hidden flex border border-white/5">
                  <div
                    ...
                    style={{
                      width: `${(regressionHealth.passed / regressionHealth.total) * 100}%`,
                    }}
                  />
                  <div
                    className="h-full bg-status-error"
                    style={{
                      width: `${(regressionHealth.failed / regressionHealth.total) * 100}%`,
                    }}
                  />
                  <div
                    className="h-full bg-status-warning"
                    style={{
                      width: `${(regressionHealth.pending / regressionHealth.total) * 100}%`,
                    }}
                  />
                </div>
                <div className="flex items-center gap-2">
                  <div className="h-1.5 w-1.5 rounded-none bg-status-error" />
                  <span className="text-[8px] font-black uppercase text-white/40 tracking-[0.2em]">
                    PROTOCOL_STATE:{' '}
                    <span className="text-status-error">
                      {regressionHealth.status}
                    </span>
                  </span>
                </div>
              </div>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 py-2">
          {teamSignals?.map((sig, idx) => (
            <div
              key={idx}
              className="p-3 bg-white/5 border-l-2 border-primary group hover:bg-white/10 transition-colors rounded-none"
            >
              <div className="flex justify-between items-start mb-3">
                <span
                  className={`text-[8px] font-black uppercase tracking-[0.2em] ${sig.priority === 'critical' ? 'text-status-error' : 'text-primary'}`}
                >
