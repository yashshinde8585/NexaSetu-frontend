    ...
    <div className="min-h-screen bg-background text-text p-4 lg:p-6 font-sans selection:bg-primary max-w-screen-2xl mx-auto flex flex-col gap-6">
      ...
      >
        ...
        <MetricStripItem
          ...
          color={
            controlStrip.failed > 0 ? 'text-status-error' : 'text-white/40'
          }
        ...
        <MetricStripItem
          ...
          color={
            controlStrip.blocked > 0 ? 'text-status-warning' : 'text-white/40'
          }
                ...
                <tbody className="divide-y divide-white/[0.02]">
                    ...
                    >
                      <td className="py-3 px-3">
                        <div className="flex flex-col gap-1 pr-6">
                          <span className="text-[10px] font-black text-white uppercase tracking-widest group-hover:text-primary transition-colors leading-tight">
                            {item.title}
                          </span>
                          {item.blocked && (
                            <span className="text-[8px] text-status-warning font-black uppercase tracking-[0.2em] leading-none italic">
                              BLOCKED_BY: {item.blockedBy}
                            </span>
                          )}
                        </div>
                      </td>
                      <td className="py-3 px-3 text-[9px] font-black text-white/30 tracking-[0.2em] uppercase tabular-nums">
                        {item.module}
                      </td>
                      <td className="py-3 px-3">
                  ...
                  {(!executionBoard || executionBoard.length === 0) && (
                    <tr>
                      <td
                        colSpan="5"
                        className="py-12 text-center text-[9px] text-white/10 uppercase font-black tracking-[0.3em] italic"
                      >
                        ZERO_EXECUTION_UNITS
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <DashboardSection title="Bug Tracker" icon={<Bug size={14} />}>
              <div className="flex flex-col gap-px bg-white/5 border border-white/10 rounded-none mt-2 overflow-hidden">
                {bugTracker?.map((bug, idx) => (
                  ...
                  >
                    <div className="flex flex-col gap-1">
                      <span className="text-[10px] font-black text-white uppercase tracking-widest group-hover:text-primary transition-colors leading-none truncate pr-4">
                        {bug.title}
                      </span>
                      <span className="text-[8px] font-black uppercase text-status-error/40 tracking-[0.2em] leading-none">
                ...
                {(!bugTracker || bugTracker.length === 0) && (
                  <div className="py-12 text-center text-[9px] text-white/10 uppercase font-black tracking-widest italic border-white/10">
                    CLEAN_SHEET
                  </div>
                )}
              </div>
            ...
            >
              <div className="flex flex-col gap-px bg-white/5 border border-white/10 rounded-none mt-2 overflow-hidden max-h-[320px] overflow-y-auto custom-scrollbar">
                {activity?.slice(0, 6).map((sign, idx) => (
                  <div
                    key={idx}
                    className="bg-black border-b border-white/5 last:border-0 px-3"
                  >
                    <ActivityItem text={sign.text} time={sign.time} />
                  </div>
                ))}
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          >
            <div className="flex flex-col items-center py-6 bg-white/5 rounded-none border border-white/10 gap-8 mt-2 relative overflow-hidden group">
              <div
                className={`text-2xl font-black tracking-widest leading-none transition-colors ${releaseStatus?.isReady ? 'text-status-success' : 'text-status-error'}`}
              >
              ...
              <div className="grid grid-cols-3 gap-px bg-white/10 border-y border-white/10 w-full text-center">
                <div className="bg-black py-4 flex flex-col gap-1">
                  <span className="text-xl font-black text-status-error leading-none tabular-nums tracking-widest">
                    {releaseStatus?.criticalBugs}
                  </span>
                  <span className="text-[8px] font-black text-white/20 uppercase tracking-widest leading-none italic">
                    BUGS
                  </span>
                </div>
                <div className="bg-black py-4 flex flex-col gap-1">
                  <span className="text-xl font-black text-status-error leading-none tabular-nums tracking-widest">
                    {releaseStatus?.failedTests}
                  </span>
                  <span className="text-[8px] font-black text-white/20 uppercase tracking-widest leading-none italic">
                    FAILED
                  </span>
                ...
                <div className="bg-black py-4 flex flex-col gap-1">
                  <span className="text-xl font-black text-status-warning leading-none tabular-nums tracking-widest">
                    {releaseStatus?.blockedTests}
                  </span>
                  <span className="text-[8px] font-black text-white/20 uppercase tracking-widest leading-none italic">
                    BLOCKED
                  </span>
            ...
            <div className="flex flex-col gap-px bg-white/10 border border-white/10 rounded-none overflow-hidden mt-2">
              <div className="bg-black p-4 flex justify-between items-center group hover:bg-white/5 transition-colors">
                <div className="flex flex-col gap-1.5">
                  <span className="text-[8px] font-black uppercase tracking-[0.2em] text-white/20 leading-none">
                    ENVIRONMENT
                  </span>
                  <span className="text-[10px] font-black text-white uppercase tracking-widest leading-none">
                    {buildStatus?.environment}
                  </span>
                </div>
              ...
              <div
                className={`p-4 flex items-center justify-between border-t border-white/5 ${
                  buildStatus?.status === 'Stable'
                    ? 'bg-status-success/5'
                    : 'bg-status-error/5'
                }`}
              >
          ...
          >
            <div className="flex flex-col gap-6 py-4 px-4 bg-white/5 border border-white/10 rounded-none mt-2">
              <div className="flex items-end justify-between leading-none">
                <span className="text-3xl font-black text-white tracking-widest tabular-nums leading-none">
                  {cycleProgress?.percentage}%
                </span>
                <span className="text-[8px] font-black text-white/20 uppercase tracking-[0.2em] tabular-nums">
                  {cycleProgress?.executed} / {cycleProgress?.total} EXEC
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-3 bg-status-success/5 border border-status-success/20 rounded-none text-center group hover:bg-status-success/10 transition-colors">
                  <span className="block text-xl font-black text-status-success leading-none tabular-nums tracking-widest">
                    {cycleProgress?.passed}
                  </span>
                  <span className="text-[8px] font-black uppercase text-status-success/40 tracking-[0.2em] mt-1.5 block">
                    PASSED
                  </span>
                </div>
                <div className="p-3 bg-status-error/5 border border-status-error/20 rounded-none text-center group hover:bg-status-error/10 transition-colors">
                  <span className="block text-xl font-black text-status-error leading-none tabular-nums tracking-widest">
                    {cycleProgress?.failed}
                  </span>
                  <span className="text-[8px] font-black uppercase text-status-error/40 tracking-[0.2em] mt-1.5 block">
                    FAILED
                  </span>
                </div>
              </div>
        ...
        <DashboardSection title="Metric Coverage" icon={<Layers size={14} />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 py-4 px-4">
            {Object.entries(coverage || {}).map(([mod, val], idx) => (
              <div key={idx} className="flex flex-col gap-2 group">
                <div className="flex justify-between items-end leading-none">
                  <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.2em] group-hover:text-white transition-colors">
                    {mod}
                  </span>
                  <span
                    className={`text-[10px] font-black tabular-nums ${val > 80 ? 'text-status-success' : 'text-status-warning'}`}
                  >
                    {val}%
                  </span>
                </div>
                <div className="w-full h-0.5 bg-white/5 rounded-none overflow-hidden border border-white/5">
                  <div
                    className={`h-full transition-all duration-1000 ${val > 80 ? 'bg-status-success' : 'bg-status-warning'}`}
                    style={{ width: `${val}%` }}
                  />
                </div>
              </div>
            ...
            {(!coverage || Object.keys(coverage).length === 0) && (
              <div className="col-span-2 py-8 text-center text-[9px] text-white/10 uppercase font-black tracking-widest italic">
                AWAITING_COVERAGE_DATA
              </div>
            )}
          </div>
        </DashboardSection>
          ...
          <div className="flex flex-col gap-6 py-4 px-4 bg-white/5 border border-white/10 rounded-none mt-2">
            <div className="flex items-center justify-between">
              <div className="flex flex-col gap-2">
                <span className="text-3xl font-black text-white tracking-widest leading-none tabular-nums">
                  {regressionStatus?.passed} / {regressionStatus?.total}
                </span>
                <span className="text-[8px] font-black uppercase text-white/20 tracking-[0.2em] italic">
                  PASSED_TESTS
                </span>
              </div>
              <div className="flex gap-2">
                <div className="flex flex-col items-center px-4 py-2 bg-black border border-white/10 rounded-none group hover:border-status-error/40 transition-colors">
                  <span className="text-lg font-black text-status-error leading-none tabular-nums tracking-widest">
                    {regressionStatus?.failed}
                  </span>
                  <span className="text-[7px] font-black text-white/20 uppercase tracking-widest mt-1 whitespace-nowrap">
                    FAILED
                  </span>
                </div>
                <div className="flex flex-col items-center px-4 py-2 bg-black border border-white/10 rounded-none group hover:border-status-warning/40 transition-colors">
                  <span className="text-lg font-black text-status-warning leading-none tabular-nums tracking-widest">
            ...
            <div className="w-full h-0.5 bg-white/5 rounded-none flex overflow-hidden border border-white/5">
              <div
                ...
                style={{
                  width: `${(regressionStatus?.passed / regressionStatus?.total) * 100 || 0}%`,
                }}
              />
              <div
                className="h-full bg-status-error transition-all duration-1000"
                style={{
                  width: `${(regressionStatus?.failed / regressionStatus?.total) * 100 || 0}%`,
                }}
              />
              <div
                className="h-full bg-status-warning transition-all duration-1000"
                style={{
                  width: `${(regressionStatus?.pending / regressionStatus?.total) * 100 || 0}%`,
                }}
              />
            </div>
          </div>
