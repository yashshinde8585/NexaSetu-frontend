...
const HRDashboard = () => {
  ...
  const teamCapacity = Array.isArray(data?.teamCapacity)
    ? data.teamCapacity
    : [];
  const individualSignals = Array.isArray(data?.individualSignals)
    ? data.individualSignals
    : [];
  const burnoutRisk = Array.isArray(data?.burnoutRisk) ? data.burnoutRisk : [];
  const hiringImpact = data?.hiringImpact || {
    openRoles: [],
    impactInsight: { text: 'NO_DATA' },
  };
  const orgActivity = Array.isArray(data?.orgActivity) ? data.orgActivity : [];
  const workforceTrend = data?.workforceTrend || {
    avgLoad: { now: 0, last: 0 },
    hiringGap: { now: 0, last: 0 },
  };
  const retentionSignals = Array.isArray(data?.retentionSignals)
    ? data.retentionSignals
    : [];
  const rebalanceSuggestions = Array.isArray(data?.rebalanceSuggestions)
    ...
    <div className="min-h-screen bg-background text-text px-4 lg:px-6 py-2 lg:py-4 font-sans selection:bg-primary max-w-screen-2xl mx-auto flex flex-col gap-6">
      ...
      >
        ...
        <MetricStripItem
          ...
          color={
            workforceHealth.avgUtilization > 90
              ? 'text-status-error'
              : 'text-status-success'
          }
          accent={
        ...
        <MetricStripItem
          ...
          color={
            workforceHealth.overloadedTeams > 0
              ? 'text-status-error'
              : 'text-white/40'
          }
          accent={
        ...
        <MetricStripItem
          ...
          icon={<Heart size={14} />}
          color={
            workforceHealth.burnoutRisk === 'HIGH'
              ? 'text-status-error'
              : 'text-status-success'
          }
          accent={
                ...
                <tbody className="divide-y divide-white/5 text-[10px] font-black uppercase tracking-widest">
                  {Array.isArray(teamCapacity) &&
                    teamCapacity.map((t, idx) => (
                      ...
                      >
                        <td className="py-3 px-3 text-white group-hover:text-primary transition-colors">
                          {t.team}
                        </td>
                        ...
                        <td className="py-3 px-3">
                          <div className="flex flex-col items-center gap-1.5">
                            <div className="w-24 h-0.5 bg-white/5 rounded-none overflow-hidden border border-white/5">
                              <div
                                className={`h-full ${t.load > 100 ? 'bg-status-error' : 'bg-primary'}`}
                                style={{ width: `${Math.min(t.load, 100)}%` }}
                              />
                            </div>
                            <span
                              className={`text-[8px] font-black ${t.load > 100 ? 'text-status-error' : 'text-white/40'}`}
                            >
                              {t.load}%
                            </span>
                          </div>
                        ...
                        <td className="py-3 px-3 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <span
                              className={`text-[7px] font-black tracking-[0.2em] ${t.status === 'Overloaded' ? 'text-status-error' : 'text-status-success'}`}
                            >
                              {t.status === 'Overloaded'
                                ? 'CRITICAL'
                                : 'NOMINAL'}
                            ...
                            <div
                              className={`w-1.5 h-1.5 rounded-none ${t.status === 'Overloaded' ? 'bg-status-error' : 'bg-status-success'}`}
                            />
                          </div>
                        </td>
                      </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <div className="flex flex-col gap-2">
                {Array.isArray(hiringImpact?.openRoles) &&
                  hiringImpact.openRoles.map((role, idx) => (
                    <div
                      key={idx}
                      className="p-3 bg-white/5 border border-white/10 rounded-none flex justify-between items-center group hover:bg-white/10 transition-colors"
                    >
                      ...
                      <span
                        className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-none border ${
            ...
            >
              <div className="flex flex-col gap-2">
                {Array.isArray(rebalanceSuggestions) &&
                  rebalanceSuggestions.map((sug, idx) => (
                    <div
                      key={idx}
                      className="flex gap-3 p-3 border-l-2 border-primary bg-white/5 rounded-none group hover:bg-white/10 transition-colors justify-between items-center"
                    >
                      <div className="flex gap-3">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <span className="text-[7px] text-primary/60 font-black uppercase tracking-[0.2em]">
                            {sug.priority}_PRIORITY
                          </span>
                        </div>
                      </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          >
            <div className="flex flex-col gap-3">
              {Array.isArray(burnoutRisk) &&
                burnoutRisk.map((r, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-3 bg-white/5 border border-white/10 rounded-none group hover:bg-white/10 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-1.5 rounded-none border ${r.risk === 'HIGH' ? 'bg-status-error/10 text-status-error border-status-error/20' : 'bg-status-warning/10 text-status-warning border-status-warning/20'}`}
                      >
                        <ShieldAlert size={12} />
                      </div>
                      <div className="flex flex-col">
                        ...
                        <span className="text-[7px] font-black uppercase text-white/20 tracking-[0.2em] leading-none">
                          {r.risk}_RISK
                        </span>
                      </div>
                    </div>
              ...
              <div className="mt-2 p-3 bg-primary/5 border border-primary/10 rounded-none flex items-start gap-3">
                ...
                <div className="flex flex-col gap-1">
                  <h4 className="text-[8px] font-black text-primary uppercase tracking-[0.2em] leading-none">
                    AI_STRUCTURAL_INSIGHT
                  </h4>
          ...
          >
            <div className="flex flex-col gap-6 py-2 px-1">
              <TrendIndicator
                label="UTILIZATION_VARIANCE"
                now={workforceTrend?.avgLoad.now}
                last={workforceTrend?.avgLoad.last}
                unit="%"
              />
              <TrendIndicator
                label="REQUISITION_GAP"
                now={workforceTrend?.hiringGap.now}
                last={workforceTrend?.hiringGap.last}
                unit=" REQ"
                inverse
              />
            </div>
      ...
      <DashboardSection
        title="Personnel Capacity Analysis"
        icon={<TrendingUp size={14} />}
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.isArray(individualSignals) &&
            individualSignals.map((sig, idx) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`w-7 h-7 rounded-none border flex items-center justify-center text-[9px] font-black ${
                      sig.load > 100
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[7px] text-white/20 uppercase font-black tracking-[0.2em] leading-none">
                      {sig.team}_UNIT
                    </span>
                  </div>
                </div>
                <span
                  className={`text-[11px] font-black ${sig.load > 100 ? 'text-status-error' : 'text-white/40'}`}
                >
                  {sig.load}%
                </span>
              </div>
