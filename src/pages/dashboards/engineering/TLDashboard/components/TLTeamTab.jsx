      ...
      <div className="bg-white/[0.02] border border-white/5 p-4 flex flex-col justify-between">
        ...
        <div className="flex flex-col gap-2">
          {serviceMap?.slice(0, 5)?.map((s, idx) => (
            <div
              key={idx}
              className="flex items-center gap-3 p-3 bg-white/5 border border-white/10 rounded-none hover:bg-white/10 transition-colors"
            >
              <div className="w-8 h-8 rounded-none border border-white/10 flex items-center justify-center text-[9px] font-black text-white/40 uppercase">
                {s.owner?.[0] || ''}
              </div>
              <div className="flex flex-col gap-1 truncate pr-2">
                <span className="text-[7px] font-black text-white/20 uppercase tracking-[0.2em] leading-none">
                  OWNER: {s.name}
                </span>
                <span className="text-[10px] font-black text-white uppercase tracking-widest truncate leading-none">
                  {s.owner}
                </span>
              </div>
            </div>
