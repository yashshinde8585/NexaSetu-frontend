...
const CTODashboard = () => {
  ...
  const leadership = functionalBreakdown.engineeringLeadership || {};
  const engineering = functionalBreakdown.engineeringIC || {};
  ...
  return (
    <div className="min-h-screen bg-background text-text p-4 lg:p-6 font-sans selection:bg-primary selection:text-text max-w-screen-2xl mx-auto">
      ...
      <div
        id="cto-performance-strip"
        className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6"
      >
        <MetricStripItem
          label="Projects At risk"
          value={global.projectsAtRisk}
          color="text-status-error"
          accent="bg-status-error"
        />
        <MetricStripItem
          label="Active Blockers"
          value={global.totalBlockers}
          color="text-status-warning"
          accent="bg-status-warning"
        />
        <MetricStripItem
          label="Avg Drift"
          value={`+${global.orgDeliveryDrift}d`}
          color="text-primary"
          accent="bg-primary"
        />
        <MetricStripItem
          label="Energy/Cap Usage"
          value={`${global.avgUtilization}%`}
          color="text-status-primary"
          accent="bg-primary"
        />
        <MetricStripItem
          label="Teams At capacity"
          value={global.teamsOverloaded}
          color="text-status-warning"
          accent="bg-status-warning"
        />
      </div>
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FunctionalCard
              ...
              metrics={[
                { label: 'Active Projects', value: leadership.activeProjects },
                {
                  label: 'Delayed',
                  value: leadership.delayed,
                  color: 'text-status-error',
                },
                {
                  label: 'Avg Delay',
                  value: `+${leadership.avgDrift}d`,
                  color: 'text-white',
                },
                {
                  label: 'Pending Approvals',
                  value: leadership.decisionsPending,
                  color: 'text-status-warning',
                },
              ]}
            ...
            <FunctionalCard
              ...
              metrics={[
                { label: 'Engineers', value: engineering.totalEngineers },
                {
                  label: 'Workload',
                  value: `${engineering.avgLoad}%`,
                  color: 'text-white',
                },
                {
                  label: 'Overloaded',
                  value: engineering.overloaded,
                  color: 'text-status-error',
                },
                { label: 'Velocity', value: `${engineering.completionRate}%` },
              ]}
              ...
              <table className="w-full text-left border-separate border-spacing-0">
                <thead>
                  <tr className="text-white/40 text-[9px] uppercase font-black tracking-[0.2em] border-b border-white/10">
                    <th className="py-3 px-4 border-b border-white/5">
                      DEPARTMENT
                    </th>
                    <th className="py-3 px-4 border-b border-white/5 text-center">
                      WORKLOAD %
                    </th>
                    <th className="py-3 px-4 border-b border-white/5 text-center">
                      BLOCKERS
                ...
                <tbody className="text-[10px] font-black uppercase tracking-widest">
                  {Array.isArray(functionalTable) &&
                    functionalTable.map((row, i) => (
                      <tr
                        key={i}
                        onClick={() =>
                          handleDrilldown(
                            row.function?.split(' ')[0] || 'engineering'
                          )
                        }
                        className="hover:bg-white/[0.02] transition-colors cursor-pointer group group/row"
                      >
                        <td className="py-3 px-4 border-b border-white/[0.03] group-hover/row:text-primary transition-colors">
                          {row.function}
                        </td>
                        <td className="py-3 px-4 text-center text-white border-b border-white/[0.03]">
                          {row.load}%
                        </td>
                        <td className="py-3 px-4 text-center text-white border-b border-white/[0.03]">
                          {row.blockers}
                        </td>
                        <td className="py-3 px-4 text-white/60 border-b border-white/[0.03]">
                          {row.delay}
                        </td>
              ...
              {(() => {
                ...
                return (
                  <div className="flex flex-col gap-6">
                    <PipelineStep
                      label="Backlog"
                      value={pipeline.backlog}
                      progress={getProgress(pipeline.backlog)}
                      color="bg-white/10"
                    />
                    <PipelineStep
                      label="In Development"
                      value={pipeline.dev}
                      progress={getProgress(pipeline.dev)}
                      color="bg-primary"
                    />
                    <PipelineStep
                      label="In QA Review"
                      value={pipeline.qa}
                      progress={getProgress(pipeline.qa)}
                    ...
                    <PipelineStep
                      ...
                      progress={getProgress(pipeline.release)}
