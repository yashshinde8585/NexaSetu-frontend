...
// Processes secure workspace invitations by verifying tokens and finalizing new user account registration.
const Join = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        if (
          activatedUser.role === 'INTERN' &&
          activatedUser.assignedProjectId
        ) {
      ...
      <div className="max-w-md w-full bg-background border border-white/5 p-8 rounded-3xl shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10 text-center mb-8">
          ...
          <p className="text-text-muted text-sm px-4">
            ...
            <span className="text-primary font-bold">{inviteData.role}</span>
          </p>
        </div>
        ...
        {status === 'success' ? (
          ...
          >
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-text-muted/60 ml-1">
                Work Email
              </label>
              <div className="relative group opacity-60">
                ...
                <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block w-48 p-2 bg-background-light border border-white/10 rounded-lg text-[9px] font-bold text-white shadow-2xl z-50">
                  Invitations are locked to the recipient's email for
                  governance.
                </div>
              </div>
            </div>
