                    ...
                    <tr key={i} 
                      ...
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-3">
                          <span className="text-white group-hover:text-primary transition-colors">{row.team}</span>
                          {row.blockers > 0 && <span className="text-[7px] text-status-error tracking-[0.2em] font-black">{row.blockers}_BLOCKED</span>}
                        </div>
                      </td>
                      <td className="py-3 px-3">
                      ...
                      <td className="py-3 px-3 text-center">
                        <span className={`px-2 py-0.5 rounded-none border text-[8px] font-black tracking-[0.2em] ${parseInt(row.load) > 100 ? 'text-status-error border-status-error/30 bg-status-error/5' : 'text-white/40 border-white/10 bg-white/5'}`}>
                          {row.load}
                        </span>
                      </td>
              ...
              <div className="flex flex-col gap-6 pt-2">
                <div className="flex justify-between items-end leading-none">
                  <div className="flex flex-col gap-2">
                    <span className="text-[8px] text-white/40 uppercase font-black tracking-[0.2em]">AGGREGATE_PROGRESS</span>
                    <div className="text-2xl font-black text-white tracking-widest">{sprintExecution?.progress || 0}%</div>
                  </div>
                  <div>
                ...
                <div className="w-full h-1 bg-white/5 rounded-none overflow-hidden border border-white/5">
                  <div className="h-full bg-primary transition-all duration-1000" style={{ width: `${sprintExecution?.progress || 0}%` }} />
                </div>
                ...
                <div className="grid grid-cols-3 gap-px bg-white/10 border border-white/10 rounded-none overflow-hidden">
                  ...
                  <SprintCounter label="ACTIVE" value={sprintExecution?.inProgress || 0} color="text-primary" />
          ...
          <DashboardSection title="Top Blockers" icon={<ShieldAlert size={14} />}>
            <div className="flex flex-col gap-px bg-white/10 border border-white/10 rounded-none overflow-hidden">
              {Object.keys(blockers).map((teamName, idx) => (
                <div key={idx} className="bg-black p-4 border-b border-white/[0.05] last:border-0 flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                    {blockers[teamName].map((item, i) => (
                      <div key={i} className="text-[9px] text-white/60 font-black uppercase tracking-widest flex items-start gap-2 p-3 bg-white/5 border border-white/10 rounded-none group hover:bg-white/10 transition-colors leading-snug">
                        <span className="text-status-error shrink-0 font-black">::</span>
                        {item}
                      </div>
                    ))}
                  </div>
              ...
              {Object.keys(blockers).length === 0 && (
                <div className="py-12 text-center opacity-20 border border-white/10 border-dashed rounded-none">
                  <span className="text-[8px] uppercase font-black tracking-[0.2em]">NO_BLOCKERS_REGISTERED</span>
                </div>
              )}
            </div>
          </DashboardSection>
...
const SprintCounter = ({ label, value, color }) => (
  <div className="bg-black p-3 flex flex-col gap-1.5 text-center group hover:bg-white/5 transition-colors">
    <span className="text-[7px] text-white/20 uppercase font-black tracking-[0.2em]">{label}</span>
    <span className={`text-[12px] font-black uppercase tracking-widest ${color}`}>{value}</span>
  </div>
);
...
const VelocityIndicator = ({ direction }) => {
  if (direction === 'up') return (
    <div className="flex items-center gap-1.5 text-status-success bg-status-success/5 px-2 py-1 rounded-none border border-status-success/30">
      <TrendingUp size={10} />
      <span className="text-[7px] font-black uppercase tracking-[0.2em]">IMPROVING</span>
    </div>
  );
  if (direction === 'down') return (
    <div className="flex items-center gap-1.5 text-status-error bg-status-error/5 px-2 py-1 rounded-none border border-status-error/30">
      ...
      <span className="text-[7px] font-black uppercase tracking-[0.2em]">DECLINING</span>
    </div>
  );
  return (
    <div className="flex items-center gap-1.5 text-white/40 bg-white/5 px-2 py-1 rounded-none border border-white/10">
      <Minus size={10} />
      <span className="text-[7px] font-black uppercase tracking-[0.2em]">STABLE</span>
    </div>
  );
};
