...
import { sanitizeInput, validateInviteForm } from '../../utils/validation';
...
const AddTeamMember = () => {
      ...
      ? [
          {
            ...
            roles: customRoles
              .filter((r) => r.isCustom)
              .map((r) => ({ title: r.role.replace(/_/g, ' '), role: r.role })),
          },
        ]
      : []),
  ];
  ...
  const handleSendAll = async (e) => {
    ...
    const validationError = validateInviteForm(validInvites);
    if (validationError) {
      setError(validationError);
      return;
    }

    const sanitizedInvites = validInvites.map((inv) => ({
      ...inv,
      name: sanitizeInput(inv.name),
      email: inv.email.trim(),
    }));

    setInvites(sanitizedInvites);

    setStatus('loading');
    setError('');

    try {
      const res = await TeamService.inviteBulk({ invites: sanitizedInvites });
      setResults(res);
      setStatus('success');
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 border-b border-white/10 pb-6">
        <div className="space-y-1">
          ...
          <p className="text-white/40 text-[9px] font-black uppercase tracking-[0.2em] max-w-xl">
            Expand your workspace by inviting new personnel and assigning
            tactical roles.
          </p>
        </div>
      </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="relative">
                    <TacticalCustomSelect
                      ...
                      icon={<Rocket size={14} />}
                      displayValue={
                        projects.find((p) => p._id === invite.projectId)?.name
                      }
