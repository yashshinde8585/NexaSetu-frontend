...
const ApprovalPanel = ({ actions, handleApprove, handleReject }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-primary/10 to-transparent border border-primary/20 rounded-[3rem] p-8 shadow-2xl relative overflow-hidden group">
      <div className="absolute top-0 right-0 p-10 text-8xl opacity-5 group-hover:scale-125 transition-transform duration-1000">⚡</div>
      <div className="flex items-center gap-3 mb-8">
        <span className="p-3 bg-primary text-white rounded-2xl shadow-lg shadow-primary/20 text-xl font-bold">Pending Execution</span>
        <div>
          <h2 className="text-xl font-black text-white">Strategic Approvals Required</h2>
          <p className="text-xs text-primary font-bold uppercase tracking-widest">AI requires your oversight for high-impact changes</p>
        </div>
      </div>
        ...
        {actions.map(action => (
          <div key={action._id} className="bg-background-dark/60 border border-white/5 p-6 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-center gap-6 group/item hover:border-primary/30 transition-all">
            <div className="flex-1 space-y-2">
              ...
              <h4 className="text-lg font-bold text-white leading-tight">Proposed Change: {action.reason}</h4>
              <p className="text-xs text-text-muted">Target: <span className="text-white font-bold">{action.targetId?.title || action.targetId?.name || 'Loading...'}</span></p>
            </div>
