...
// A component that presents AI-driven optimization recommendations and strategic advice for projects.
const SuggestionPanel = ({ recommendations, setRecommendations }) => {
  ...
  return (
    <div className="bg-background-dark/40 border border-white/5 rounded-[3rem] p-8 shadow-2xl relative overflow-hidden">
      <div className="absolute top-0 right-0 p-10 text-8xl opacity-5 pointer-events-none">
        🧠
      </div>
      <div className="flex items-center gap-3 mb-6">
        <span className="p-2 bg-primary/20 rounded-lg text-primary text-xl">
          🤖
        </span>
        <div className="flex flex-col">
          ...
          <p className="text-[10px] text-text-muted font-bold tracking-widest uppercase">
            Real-time Project Optimization
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {recommendations.slice(0, 6).map((rec, idx) => (
          <div
            key={idx}
            className="bg-background-light/40 border border-white/5 p-6 rounded-[2rem] hover:border-primary/20 transition-all group flex flex-col justify-between"
          >
            <div>
              <div className="flex justify-between items-start mb-3">
                <span
                  className={`text-[8px] font-black uppercase tracking-tighter px-2 py-0.5 rounded-full border ${
                    rec.priority === 'high'
