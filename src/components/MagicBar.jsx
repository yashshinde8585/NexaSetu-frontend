...
const MagicBar = () => {
  const { user } = useAuth();
  const { setGlobalResult, pendingCommand, setPendingCommand, activeProjects, dashboardContext } = useMagic();
  ...
  const getSuggestions = () => {
    ...
    const projectCommands = activeProjects?.flatMap(p => [
      { id: `proj-opt-${p._id}`, label: `Optimize ${p.name}`, prompt: `Optimize path for ${p.name}`, type: 'project' },
      { id: `proj-health-${p._id}`, label: `${p.name} Health`, prompt: `Show health status for ${p.name}`, type: 'project' },
      { id: `proj-tasks-${p._id}`, label: `${p.name} Tasks`, prompt: `List incomplete tasks for ${p.name}`, type: 'project' }
    ]) || [];
  ...
  const executeCommand = async (commandText) => {
    ...
    try {
      // Send query with rich frontend context (stats, active projects)
      const res = await executeMagicCommand(activeQuery, {
          stats: dashboardContext?.stats,
          visibleProjects: activeProjects.map(p => ({ name: p.name, health: p.healthScore, percentage: p.percentage }))
      });
  ...
  return (
    <div ref={paletteRef} className="relative w-full transition-all duration-300">
      <form 
      ...
      >
        <div className={`relative flex items-center h-full px-4 w-full`}>
              <span className="text-[12px] mr-2 opacity-60 hover:scale-110 transition-transform cursor-default">✨</span>
              <input
                ref={inputRef}
                type="text"
                value={query}
            ...
            {isProcessing ? (
                ...
                <div className="flex items-center ml-3 gap-3">
                    ...
                    <button type="submit" className="text-white hover:text-white hover:scale-110 transition-all bg-primary/20 hover:bg-primary/30 p-1.5 rounded-full flex items-center justify-center -mr-2">
                      <svg className="w-3.5 h-3.5 stroke-primary-light" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24">
      ...
      {isFocused && (query.trim().length > 0) && filteredSuggestions.length > 0 && (
        <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-[#161622]/95 backdrop-blur-2xl border border-white/10 rounded-2xl py-2 shadow-2xl animate-in fade-in slide-in-from-top-2 duration-300 z-[60] overflow-hidden">
            {filteredSuggestions.map((s, index) => (
                <button
                    key={s.id}
                    onMouseDown={(e) => { e.preventDefault(); handleSuggestionClick(s.prompt); }}
                    className={`w-full text-left px-4 py-3 transition-all group border-b border-white/[0.02] last:border-0 flex items-center justify-between ${
                      selectedIndex === index ? 'bg-white/[0.08] translate-x-1' : 'hover:bg-white/[0.04]'
                    }`}
                >
                    <div className="flex flex-col">
                        <span className={`text-[12px] font-bold group-hover:text-primary transition-colors flex items-center gap-3 ${
                          selectedIndex === index ? 'text-primary' : 'text-white/90'
                        }`}>
                           <span className={`w-1.5 h-1.5 rounded-full transition-all ${
                             selectedIndex === index ? 'bg-primary animate-pulse' : 'bg-primary/40'
                           }`}></span>
                           {s.label}
                        </span>
                        <span className="text-[10px] text-text-muted/60 font-medium ml-4.5">{s.prompt}</span>
                    </div>
                    {selectedIndex === index && (
                      <span className="text-[10px] text-primary/40 flex items-center gap-1 font-black uppercase tracking-widest animate-in fade-in slide-in-from-right-2">
                        Enter
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M11 5L18 12L11 19M6 12H18" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round"/></svg>
                      </span>
                    )}
                </button>
            ))}
