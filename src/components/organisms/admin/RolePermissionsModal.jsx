...
const RolePermissionsModal = ({ isOpen, onClose, role, availablePermissions, onSave }) => {
      ...
      <div className="relative bg-[#0A0A0A] border border-white/10 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-primary/10 to-transparent">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Manage Access: {role.role}</h2>
              <p className="text-xs text-white/40 mt-0.5">Define granular permissions for this organizational role</p>
            </div>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className={`text-sm font-semibold mb-1 ${isActive ? 'text-primary' : 'text-white/70'}`}>
                        {perm.replace(/_/g, ' ')}
                      </p>
        ...
        <div className="px-8 py-6 bg-white/2 border-t border-white/5 flex items-center justify-between">
          <p className="text-xs text-white/30">
            <span className="text-primary font-bold">{selectedPermissions.length}</span> permissions assigned
          </p>
