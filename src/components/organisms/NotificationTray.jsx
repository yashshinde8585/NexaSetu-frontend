...
const NotificationTray = () => {
  ...
  return (
    <div className="relative h-full flex items-center" ref={trayRef}>
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%+4px)] right-0 w-80 sm:w-85 bg-black border border-white/15 rounded-none overflow-hidden animate-in fade-in slide-in-from-top-1 duration-200 z-50">
          <div className="p-3 border-b border-white/10 flex items-center justify-between bg-white/5">
            <div className="flex items-center gap-2">
              <span className="text-[9px] font-black text-white/40 uppercase tracking-[0.2em]">Operational_Alerts</span>
              {unreadCount > 0 && <div className="w-1.5 h-1.5 rounded-none bg-primary" />}
            </div>
            ...
            {notifications.length > 0 ? (
              notifications.map((notif) => (
                ...
                >
                  ...
                  <div className={`shrink-0 w-8 h-8 rounded-none flex items-center justify-center transition-colors ${!notif.isRead ? 'bg-primary/20 border border-primary/40 text-primary' : 'bg-white/5 border border-white/10 text-white/20'}`}>
                    {getIcon(notif.type)}
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <h4 className={`text-[10px] font-black tracking-widest uppercase truncate ${!notif.isRead ? 'text-white' : 'text-white/40'}`}>
                        {notif.title}
                      </h4>
                    ...
                    <p className={`text-[9px] leading-tight line-clamp-2 uppercase font-black tracking-widest ${!notif.isRead ? 'text-white/60' : 'text-white/20'}`}>
                      {notif.message}
                    </p>
                  </div>
                </div>
              ...
              <div className="p-12 text-center flex flex-col items-center justify-center gap-4 opacity-10">
                ...
                <div className="flex flex-col gap-1">
                    <p className="text-[10px] font-black uppercase tracking-[0.3em]">ALL_CLEAR</p>
                    <p className="text-[8px] font-black uppercase tracking-[0.2em]">MISSION_LOG_SYNCHRONIZED</p>
                </div>
              </div>
            )}
          </div>
