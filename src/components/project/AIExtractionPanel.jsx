  ...
  return (
    <div className="space-y-8">
      <div className="bg-[#1A1A2E]/50 p-6 rounded-2xl mb-8 shadow-2xl border border-white/5 backdrop-blur-xl">
        <h3 className="text-[10px] font-black uppercase tracking-[0.3em] mb-4 text-primary">AI Task Generation</h3>
        <div className="space-y-4">
          ...
          <button onClick={handleAiExtract} disabled={isAiProcessing || !aiInput.trim()} className="w-full bg-primary hover:bg-primary-dark disabled:bg-white/5 disabled:text-text-muted/30 text-white font-black py-4 px-6 rounded-xl transition duration-500 flex justify-center items-center gap-3 text-[10px] uppercase tracking-widest shadow-lg shadow-primary/20">
            {isAiProcessing ? <><div className="animate-spin h-3.5 w-3.5 border-2 border-white/30 border-t-white rounded-full"></div> Processing...</> : <><span className="text-base text-white/50">✨</span> Generate Task</>}
          </button>
        </div>
      </div>
        ...
        <div className="bg-[#1A1A2E]/80 backdrop-blur-2xl p-8 rounded-[2rem] mb-8 border border-primary/30 shadow-[0_30px_60px_rgba(0,0,0,0.4)] animate-in fade-in zoom-in-95 duration-500">
          <div className="flex justify-between items-center mb-8 pb-4 border-b border-white/5">
            <h3 className="text-sm font-black flex items-center gap-3 uppercase tracking-[0.2em] text-white">
              <span className="p-2 bg-primary/10 rounded-lg text-primary">🤖</span> AI Recommendation
            </h3>
            <span className="text-[9px] font-black uppercase tracking-[0.2em] bg-primary/10 text-primary px-3 py-1.5 rounded-full border border-primary/20">Review Details</span>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="space-y-6">
              <div className="group">
                <label className="block text-[9px] font-black text-text-muted uppercase tracking-[0.2em] mb-2 ml-1 group-focus-within:text-primary transition-colors">Task Title</label>
              ...
              <div className="group">
                <label className="block text-[9px] font-black text-text-muted uppercase tracking-[0.2em] mb-2 ml-1 group-focus-within:text-primary transition-colors">Assignee</label>
                <select 
              ...
              <div className="group">
                <label className="block text-[9px] font-black text-text-muted uppercase tracking-[0.2em] mb-2 ml-1 group-focus-within:text-primary transition-colors">Sprint</label>
                <select 
                  className="w-full bg-[#0F0F13] border border-white/5 text-xs font-bold text-white px-5 py-3 rounded-xl focus:ring-primary/20 focus:ring-1 focus:outline-none focus:border-primary/30 appearance-none cursor-pointer" 
            ...
            <div className="group">
              <label className="block text-[9px] font-black text-text-muted uppercase tracking-[0.2em] mb-2 ml-1 group-focus-within:text-primary transition-colors">Task Description</label>
          ...
          <div className="flex gap-4 mt-8">
            <button onClick={() => handleCreateTask(aiSuggestion)} className="flex-1 bg-primary hover:bg-primary-dark text-white font-black py-4 px-6 rounded-xl transition-all shadow-lg active:scale-95 text-[10px] uppercase tracking-widest shadow-primary/20">✓ Create Task</button>
