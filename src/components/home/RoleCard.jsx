...
/**
 * RoleCard Component
 * Highlights value propositions for specific engineering roles.
 */
    ...
    >
        <div className="flex-grow">
            <div className="flex items-center gap-5 mb-8">
                <div className={`p-4 ${important ? 'bg-primary/20 border-primary/30' : 'bg-white/10 border-white/15'} border rounded-2xl`}>
                    {React.cloneElement(icon, { 'aria-hidden': 'true', size: 24 })}
                </div>
                <div>
                    <h4 className="font-bold text-white text-lg">{role}</h4>
                    <p className="text-[10px] font-bold text-primary uppercase tracking-[0.2em] mt-1">{highlight}</p>
                </div>
            </div>
            <p className="text-white/65 text-sm leading-relaxed mb-8">{description}</p>
        </div>
