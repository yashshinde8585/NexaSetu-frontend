...
import epiApi from '../../api/epiApi';
import StakeholderReportModal from './StakeholderReportModal';

// Project health summary component.
const ProjectHealthSummary = ({ sprintId, manualTrigger = false }) => {
  ...
  const {
    ...
    queryFn: () => epiApi.getSprintEPI(sprintId),
  ...
  const getHealthColor = (score) => {
    if (score >= 80)
      return 'text-status-success border-status-success/20 bg-status-success/5';
    if (score >= 50)
    ...
    <div className="relative group overflow-hidden rounded-[2rem] border border-white/5 bg-white/[0.01] backdrop-blur-3xl transition-all hover:border-primary/20">
      <div className="relative p-6 sm:p-7 flex flex-col lg:flex-row gap-8 items-start lg:items-center">
        ...
        <div className="flex-shrink-0 flex flex-col items-center gap-2">
          <div
            className={`relative w-24 h-24 rounded-full border-4 flex items-center justify-center transition-all duration-500 ${data ? getHealthColor(data.metrics.healthScore) : 'border-white/10 text-white/20'}`}
          >
            {loading ? (
              ...
              <div className="text-center">
                <span className="text-2xl font-black">
                  {Math.round(data.metrics.healthScore)}
                </span>
                <span className="text-[10px] block font-black uppercase tracking-tighter">
                  Health
                </span>
              </div>
            ) : (
          ...
          {data && (
            <div
              className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full border ${getHealthColor(data.metrics.healthScore)}`}
            >
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-start gap-4">
                <p className="text-sm text-white/40 italic">
                  Ready to analyze project signals and generate intelligence
                  narratives.
                </p>
                <button
              ...
              <p className="text-sm text-white/20 italic">
                Select a sprint to initialize EPI analysis...
              </p>
            )}
          </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-medium text-white/40">
                  ...
                  <span className="text-white font-bold">
                    {Math.round(data.metrics.completionPercentage)}%
                  </span>
                </span>
              </div>
              <div className="flex items-center gap-2">
                <AlertCircle
                  size={14}
                  className={`${data.criticalBlockers.length > 0 ? 'text-status-error' : 'text-white/40'}`}
                />
                <span className="text-[11px] font-medium text-white/40">
                  Blockers:{' '}
                  <span
                    className={`${data.criticalBlockers.length > 0 ? 'text-status-error font-bold' : 'text-white'}`}
                  >
        ...
        {data && data.suggestedActions?.length > 0 && (
          <div className="flex-shrink-0 w-full lg:w-96 space-y-3">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 ml-1">
              ...
              >
                <FileText
                  size={12}
                  className="group-hover/report:animate-bounce"
                />{' '}
                Generate Briefing
              </button>
            </div>
...
const ActionItem = ({ action, sprintId, onExecuted }) => {
  ...
  const actionMutation = useMutation({
    mutationFn: () => epiApi.executeEPIAction(action.type, action.metadata),
    onSuccess: () => {
      setDone(true);
  ...
  return (
    <div className="group/action p-4 rounded-2xl bg-white/[0.04] border border-white/5 hover:border-primary/40 transition-all flex items-center justify-between gap-4">
      <div className="space-y-1 min-w-0">
        <div className="flex items-center gap-2">
          <div
            className={`w-1 h-1 rounded-full ${action.impact === 'high' ? 'bg-status-error animate-pulse' : 'bg-primary'}`}
          />
          <span className="text-[11px] font-bold text-white truncate group-hover/action:text-primary transition-colors">
            {action.title}
          </span>
        </div>
        <p className="text-[9px] text-white/40 leading-relaxed line-clamp-1">
          {action.description}
        </p>
      </div>
      <button
...
export default React.memo(ProjectHealthSummary);
