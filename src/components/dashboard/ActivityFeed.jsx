...
// A component that displays a live feed of recent workspace activities and AI-driven insights.
    ...
    <div className="space-y-8">
      <div className="bg-background-light/10 rounded-2xl p-6 border border-white/5">
        ...
        {recentActivity.length === 0 ? (
          <p className="text-text-muted text-[10px] italic text-center py-10 border border-dashed border-white/5 rounded-lg">
          ...
          <div className="relative space-y-6 ml-2">
            ...
            {recentActivity.map((activity) => (
              <div key={activity._id} className="relative pl-6 group/item">
                <div
                  className={`absolute left-0 top-1.5 w-1.5 h-1.5 rounded-full ring-2 ring-background z-10 transition-all ${
                    activity.status === TASK_STATUS.DONE
                      ? 'bg-status-success'
                      : activity.status === TASK_STATUS.IN_PROGRESS
                        ? 'bg-status-warning'
                        : 'bg-status-info'
                  }`}
                ></div>
        ...
        <div className="mt-10 bg-linear-to-br from-secondary/10 via-primary/10 to-transparent p-5 rounded-xl border border-white/5 relative overflow-hidden group">
          <div className="relative z-10">
            <h3 className="text-sm font-black italic tracking-tighter mb-2 flex items-center gap-2 text-white/90">
              <Sparkles size={16} className="text-secondary animate-pulse" /> AI
              Insights
              <div className="relative group/tooltip inline-block">
                <div className="w-3 h-3 rounded-full border border-text-muted/30 flex items-center justify-center text-[7px] font-black text-text-muted/60 cursor-help hover:border-secondary hover:text-secondary transition-colors italic">
                  i
                </div>
            ...
            <Link
              to="/portfolio"
              className="inline-flex items-center gap-2 bg-white/5 hover:bg-white/10 px-4 py-1.5 rounded-lg text-xs font-bold text-white transition-all group/btn border border-white/5"
            >
              Portfolio Hub
              <Zap
                size={10}
                className="group-hover/btn:fill-primary transition-all"
              />
            </Link>
          </div>
