...
const StakeholderReportModal = ({ isOpen, onClose, sprintId }) => {
  ...
  const reportTypes = [
    {
      ...
      desc: 'Business value & delivery risks',
    },
    {
      id: 'TECHNICAL',
      label: 'Technical Health',
      icon: <Code size={16} />,
      desc: 'Velocity & engineering blockers',
    },
    {
      id: 'CLIENT',
      label: 'Client Briefing',
      icon: <Users size={16} />,
      desc: 'Accomplishments & roadmap alignment',
    },
  ];
  ...
  const fetchReport = async (type) => {
    ...
    setReportData(null);
    try {
      const data = await epiApi.getNarrativeReport(sprintId, type);
      setReportData(data);
    } catch (err) {
      ...
      <div className="bg-[#0D0E15] border border-white/10 w-full max-w-4xl max-h-[85vh] rounded-[2.5rem] shadow-full overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
        ...
        <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-white uppercase tracking-tight leading-none mb-1">
                AI Stakeholder Briefing
              </h2>
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-72 border-r border-white/5 p-6 space-y-3 bg-black/20">
            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] opacity-30 mb-4 px-2">
              Report Profile
            </h4>
            {reportTypes.map((type) => (
              ...
              >
                ...
                <p className="text-[9px] opacity-60 leading-relaxed font-medium">
                  {type.desc}
                </p>
              </button>
            ))}
            ...
            {loading ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-6 animate-in fade-in">
                ...
                <div className="text-center">
                  <p className="text-sm font-bold text-white tracking-widest uppercase mb-2">
                    Architecting Narrative
                  </p>
                  <p className="text-[10px] font-medium text-white/30 uppercase tracking-[0.2em]">
                    Aggregating signals for {reportType} profile...
                  </p>
                </div>
              </div>
            ) : reportData ? (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700">
                ...
                <div className="flex flex-wrap gap-4 items-center justify-between border-b border-white/5 pb-6">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-black uppercase tracking-widest text-white/60 italic">
                ...
                <div className="flex items-center justify-between pt-4">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[9px] font-bold text-status-success/40 uppercase tracking-widest">
                        Completed
                      </span>
                    ...
                    <div className="flex flex-col">
                      <span className="text-[9px] font-bold text-status-error/40 uppercase tracking-widest">
                        At Risk
                      </span>
                      <span className="text-sm font-black text-status-error">
                        {reportData.summary.delayed}
                      </span>
                    </div>
                  </div>
