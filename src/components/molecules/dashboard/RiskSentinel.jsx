      ...
      <div
        className={`relative overflow-hidden rounded-2xl border bg-black shadow-2xl ${
          alert.type === 'DEADLOCK_DETECTED'
            ? 'border-status-error/50 shadow-status-error/20'
            : 'border-status-warning/50 shadow-status-warning/20'
        }`}
      >
        {/* Progress Bar (Visual Polish) */}
        <div
          className={`absolute top-0 left-0 h-1 bg-current opacity-30 animate-progress ${
            alert.type === 'DEADLOCK_DETECTED'
              ? 'text-status-error'
              : 'text-status-warning'
          }`}
        ...
        <div className="p-5">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="block text-[10px] font-black uppercase tracking-[0.2em] opacity-40">
                  Autonomous Sentinel
                </span>
