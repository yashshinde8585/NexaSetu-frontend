...
const ProjectCard = ({ project }) => {
  const { _id, name, totalTasks, completedTasks, pendingTasks, inProgressTasks, aiGeneratedTasks, percentage, healthScore, healthLabel, riskLevel } = project;
  ...
  const progressColorClass = healthScore >= 80 ? 'bg-status-success' : (healthScore >= 50 ? 'bg-status-warning' : 'bg-status-error');
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
             ...
             <p className={`text-[10px] font-black uppercase tracking-widest ${healthUI.color}`}>{healthLabel}</p>
             <span className={`px-1.5 py-0.5 rounded text-[7px] font-black uppercase tracking-tighter border ${riskUI}`}>
                {riskLevel} Risk
             </span>
          </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-bold text-text-muted uppercase">Execution Progress</span>
            <span className={`text-[10px] font-bold ${healthUI.color}`}>{percentage}%</span>
          </div>
