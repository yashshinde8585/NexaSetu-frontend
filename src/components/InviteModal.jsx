...
import { X, Mail, Shield, Rocket, Send, Loader2, CheckCircle, AlertCircle, UserPlus, ChevronDown, ChevronUp, Plus, Trash2, User as UserIcon } from 'lucide-react';
...
const InviteModal = ({ isOpen, onClose }) => {
        ...
        if (isOpen) {
            const fetchProjectsData = async () => {
                try {
                    ...
                    setError('Failed to load project list. Assignments will be unavailable.');
                }
            };
            fetchProjectsData();
    ...
    const handleSendAll = async (e) => {
        ...
        // Identity Validation (Manual check to be extra safe before dispatch)
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-background-dark/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div className={`max-w-2xl w-full bg-background-dark border border-white/5 rounded-[32px] shadow-2xl relative overflow-hidden transition-all duration-500 transform ${status === 'success' ? 'scale-100' : 'scale-100'}`}>
                
                {/* Header */}
                <div className="p-8 pb-4 flex justify-between items-start">
                    <div>
                        <h2 className="text-2xl font-black text-white tracking-tighter mb-1 select-none">Invite to Workspace</h2>
                        <p className="text-text-muted text-[11px] font-medium tracking-tight">Send email invitations to bring your team into NexaSetu.</p>
                    </div>
                    <div className="flex flex-col items-end gap-3">
                        <button onClick={onClose} className="p-2 bg-white/5 hover:bg-white/10 rounded-xl transition-colors text-text-muted">
                            <X size={20} />
                        </button>
                ...
                {status === 'success' ? (
                    <div className="p-8 text-center animate-in zoom-in-95 duration-500">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2 tracking-tight">Squad Provisioned Successfully</h3>
                        <p className="text-text-muted text-sm mb-6 px-12">
                            Accounts have been birthed. Your team can now login immediately using their <span className="text-primary font-bold italic">Email</span> as their temporary <span className="text-primary font-bold italic">Password</span>.
                        </p>
                            ...
                            <div className="space-y-2.5 max-h-[120px] overflow-y-auto pr-2 custom-scrollbar">
                                {results?.sent?.map((res, i) => (
                                    <div key={i} className="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-white/5 group hover:bg-white/10 transition-all">
                                        ...
                                        <span className="px-2 py-0.5 bg-status-success/10 text-status-success text-[8px] font-black uppercase tracking-widest rounded-md border border-status-success/20">Active</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="mx-8 flex gap-3 mb-8">
                            ...
                            >
                                <Rocket size={14} className="text-primary" /> Copy Invite Message
                            </button>
                        ...
                        <div className="p-8 pt-4 max-h-[400px] overflow-y-auto custom-scrollbar">
                            <div className="space-y-4">
                                {invites.map((invite, idx) => (
                                    <div key={idx} className="group animate-in fade-in slide-in-from-left-4 duration-300 bg-white/[0.02] p-4 rounded-3xl border border-white/5" style={{ animationDelay: `${idx * 50}ms` }}>
                                        <div className="flex items-start gap-3">
                                            <div className="flex-1 space-y-3">
                                                <div className="relative group">
                                                    ...
                                                    />
                                                    <UserIcon className="absolute right-5 top-1/2 -translate-y-1/2 text-white/5 group-focus-within:text-primary/40 transition-colors" size={14} />
                                                </div>
                                                <div className="relative group">
                                                    <input 
                                                        type="email" 
                                                    ...
                                                    />
                                                    <Mail className="absolute right-5 top-1/2 -translate-y-1/2 text-white/5 group-focus-within:text-primary/40 transition-colors" size={14} />
                                                </div>
                                            </div>
                            ...
                            >
                                {status === 'loading' ? (
                                    ...
                                    <>
                                        ...
                                        Send {invites.filter(i => i.email.trim() !== '').length} Invite{invites.filter(i => i.email.trim() !== '').length !== 1 ? 's' : ''}
                                    </>
                                )}
