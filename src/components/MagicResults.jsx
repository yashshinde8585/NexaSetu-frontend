...
const MagicResults = () => {
    const { magicResult, showResults, closeGlobalResults, triggerCommand } = useMagic();
    ...
    return (
        <div className="w-full bg-background-dark/40 border-b border-white/5 animate-in slide-in-from-top-4 duration-500 overflow-hidden">
            <div className={`max-w-7xl mx-auto px-4 sm:px-8 lg:px-10 py-10 transition-all duration-500`}>
                <div className="bg-[#1E1E2E]/60 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-8 sm:p-12 shadow-2xl overflow-hidden relative group">
                    ...
                    <div className="flex justify-between items-center mb-10 pb-6 border-b border-white/[0.05]">
                        ...
                        <button onClick={closeGlobalResults} className="p-2 bg-white/5 hover:bg-white/10 rounded-xl text-text-muted hover:text-white transition-all group-hover:rotate-90 duration-300">
                    ...
                    {!magicResult ? (
                        ...
                        <div className="space-y-8 px-2 max-h-[60vh] overflow-y-auto custom-scrollbar pr-6">
                            {(magicResult.actions || []).map((action, actionIdx) => (
                                <div key={actionIdx} className={`${actionIdx > 0 ? 'pt-10 border-t border-white/5' : ''} space-y-6 relative`}>
                                        ...
                                        <div className="ml-14 space-y-4">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {action.results.map((item, idx) => (
                                                    <div 
                                                        key={idx} 
                                                        onClick={() => triggerCommand(`Analyze deeper telemetry for ${item.title || item.name}`)}
                                                    ...
                                                    >
                                                        <div className="flex flex-col">
                                                            <span className="text-[14px] font-bold text-white tracking-wide flex items-center gap-2 group-hover/item:text-primary transition-colors">
                                                                ...
                                                                <svg className="w-3 h-3 opacity-0 group-hover/item:opacity-100 transition-all transform group-hover/item:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M13 7l5 5m0 0l-5 5m5-5H6" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round"/></svg>
                                                            </span>
                                                            {item.assignedUser && (
                                                        ...
                                                        <span className={`text-[10px] px-4 py-2 rounded-full border border-b-2 font-black uppercase tracking-widest ${
                                                            (item.status === 'done' || item.healthLabel === 'Healthy') 
                            ...
                            {magicResult.actionTriggered && (
                                <div className="mt-10 p-6 bg-primary/5 border border-primary/10 rounded-[2rem] text-primary text-sm font-bold flex items-center gap-4 shadow-inner overflow-hidden relative">
                                    ...
                                    <span className="relative flex-1 tracking-wide">⚡ Strategic optimization staged. Deployment pending executive oversight.</span>
                                </div>
                            )}
                            ...
                            <div className="mt-12 pt-8 border-t border-white/5">
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {[
                                        { label: 'Optimize Overall Portofolio', cmd: 'Optimize project resource distribution globbaly' },
                                        { label: 'Fix Mission Delays', cmd: 'Fix all stalled project bottlenecks' },
                                        { label: 'Audit Team Bandwidth', cmd: 'Show team capacity and suggest reassignments' }
                                    ].map((rec, i) => (
                                        <button 
                                            key={rec.label}
